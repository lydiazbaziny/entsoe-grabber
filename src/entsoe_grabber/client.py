...
The platform serves market data as XML documents from a single endpoint, chosen
entirely by query parameters. This module handles the transport concerns --
authentication, timeouts, retries and rate-limit responses -- and returns
validated XML bytes. Reading the content itself is done elsewhere.
...
class EntsoeRateLimitError(EntsoeTransientError):
    ...
    ENTSO-E allows 400 requests per minute, counted per security token and not
    per IP address. The R3 API removed IP-based limiting, so everything that
    shares a token shares one budget. Going over it can block the token for
    about ten minutes, which is longer than any reasonable Lambda timeout.

    The client therefore retries only when the platform sent a ``Retry-After``
    value short enough to wait out inside the budget. Otherwise it gives up at
    once. A retry without that value cannot outlast a block, and each one adds
    to the request count that caused the block.
    """
...
def _summarize(content: bytes) -> str:
...
def _acknowledgement_reason(
    ...
    tuple or None
        ...
        acknowledgement but carries no readable reason. The document is
        identified by its parsed root element name, so the result does not
        depend on the encoding, on leading comments, or on text that happens
        to mention another document type.
    """
...
def _check(response: requests.Response) -> bytes:
    ...
    # The status is checked before the body on purpose. Reason code 999 has
    # more than one meaning: the guide documents it as "No matching data
    # found", but a rejected token also comes back as 401 carrying that same
    # 999, with the text "Authentication failed." Reading the body first would
    # treat a bad token as an empty result, and a scheduled run would then
    # write nothing every day without ever raising an error.
    ...
    if (
        ...
        raise EntsoeServerError(f"HTTP {status}: {_summarize(response.content)}")
    ...
    if status != HTTPStatus.OK:
        ...
        raise EntsoeRequestError(
            f"unexpected HTTP {status}: {_summarize(response.content)}"
        )
    ...
    if content_type.lower() == "text/html":
        raise EntsoeServerError(
            f"HTTP 200: unexpected HTML response: {_summarize(response.content)}"
        )
...
class EntsoeClient:
    ...
    several. :meth:`get` normalizes both forms to an immutable tuple of XML
    byte strings. Archives are read in memory, and no response is returned
    unless every document is well-formed XML.
    ...
    total_timeout
        ...
        the call. Requests and backoff both consume it, and once it is spent no
        further retry is started. A request already in flight is not
        interrupted, and a successful response is accepted even if it arrives
        after the budget is spent. Adding the socket timeouts to this budget
        does not give an upper bound on how long a call can take. The Lambda
        timeout is the hard limit for the complete invocation.
    """
        ...
        for attempt in range(1, self._max_attempts + 1):
            try:
                ...
                if (
                    ...
                    # blocked, which lasts about ten minutes. Waiting a few
                    # seconds cannot outlast that, and every retry adds to the
                    # request count that caused the block.
                    logger.error("giving up: %s (no Retry-After to honor)", error)
                    raise
    ...
    def _request_once(self, query: Mapping[str, str]) -> XmlDocuments:
        ...
        try:
            ...
            # of its exceptions. Build a sanitized replacement here, then raise
            # it after leaving the except block, so the original does not
    ...
    def _retry_delay(self, attempt: int, error: EntsoeTransientError) -> float:
        """Seconds to wait before ``attempt`` + 1, honoring ``Retry-After``."""
        if error.retry_after is not None:
            # Floored at the backoff base. A header value of 0, or a date that
            # has already passed, would otherwise send the next request with no
            # pause at all.
            return max(error.retry_after, self._backoff_base)
        ceiling = min(self._backoff_max, self._backoff_base * 2.0 ** (attempt - 1))
        # Full jitter. This spreads concurrent invocations out, instead of
        # synchronizing their retries into a second burst.
