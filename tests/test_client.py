...
def test_rate_limit_without_retry_after_gives_up_at_once(sleeps: list[float]) -> None:
    # A plain 429 most likely means the token is blocked for about ten
    # minutes. Waiting a few seconds cannot outlast that, and each retry adds
    # to the per-token count that caused the block.
    for _ in range(3):
...
def test_retry_after_zero_still_pauses(sleeps: list[float]) -> None:
    # Floored at the backoff base: honoring a literal 0 would fire the next
