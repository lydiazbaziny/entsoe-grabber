...
IOP holding data. IOP holds little data and its contents change, so the
data-dependent tests skip instead of failing when a query comes back empty. An
empty result is the platform answering correctly, not the client breaking.
"""
...
# Namespace of the generation/load responses, taken from the targetNamespace of
# ENTSO-E's published iec62325-451-6-generationload.xsd.
GENERATION_LOAD_NS = "urn:iec62325.351:tc57wg16:451-6:generationloaddocument:3:0"

# A token of the right shape that the platform will not recognize.
...
def test_rejected_token_raises_auth_error() -> None:
    """An unrecognized token fails as an auth error, not an empty result.
...
def test_fetches_a_well_formed_market_document(client: EntsoeClient) -> None:
    ...
    Asserting the namespace detects a schema change: if the platform moves to a
    new schema version, this test fails instead of the collector quietly
...
def test_a_live_document_serializes_to_rows(client: EntsoeClient) -> None:
    """A real response flattens into one row per interval, with its quantity.

    The serializer is driven by document structure alone, so the one failure
    offline tests cannot catch is a live document nesting differently from the
    fixtures. A query for realized generation reaches Point, so these rows also
    confirm that the deepest level is discovered rather than assumed. This is
    the only test that sees real platform output.
    """
    with client:
        documents = _fetch(
            client,
