...
def test_an_unfilled_period_gets_a_row_beside_a_filled_one_of_its_name() -> None:
    # Available_Period becomes a record because it contains points, and stays
    # one for the whole document. The sibling that contains none therefore gets
    # a row of its own, instead of numbered columns repeated onto the rows of
    # the filled one.
    outage = (
        b'<?xml version="1.0" encoding="UTF-8"?>'
